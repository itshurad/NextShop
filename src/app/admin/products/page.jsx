...
export default function AdminProductsPage() {
  ...
  // تغییر نام متغیر به isPending برای سازگاری کامل با React Query v5
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      ...
      toast.success(message || "محصول با موفقیت حذف شد");
      queryClient.invalidateQueries({ queryKey: ["get-products"] });
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "خطایی در حذف محصول رخ داد",
      );
    }
  };
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="border-accent/20 from-accent/15 via-accent/5 relative overflow-hidden rounded-[32px] border bg-linear-to-br to-transparent p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="text-foreground text-4xl font-black">محصولات</h1>
      ...
      {isLoading ? (
        ...
        <div className="border-border bg-surface rounded-[32px] border p-20 text-center">
        ...
        <div className="border-border bg-surface overflow-hidden rounded-[32px] border p-2 shadow-xl shadow-black/5">
          <div className="p-6">
            <h2 className="text-foreground text-lg font-black">لیست محصولات</h2>
