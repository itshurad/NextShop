...
export default function AdminCouponsPage() {
  ...
  // استفاده از isPending در ریکت کوئری نسخه ۵
  ...
  const handleDeleteCoupon = async (couponId) => {
    try {
      ...
      toast.success(message || "کد تخفیف با موفقیت حذف شد");
      queryClient.invalidateQueries({ queryKey: ["get-coupons"] });
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "خطایی در حذف کد تخفیف رخ داد",
      );
    }
  };
      ...
      <div className="border-accent/20 from-accent/15 via-accent/5 relative overflow-hidden rounded-[32px] border bg-linear-to-br to-transparent p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="text-foreground text-4xl font-black">کدهای تخفیف</h1>
            <p className="text-muted mt-3 max-w-xl text-sm">
              مدیریت، ویرایش و مشاهده وضعیت کدهای تخفیف فعال و منقضی شده
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div className="border-warning/10 from-warning/15 rounded-[28px] border bg-linear-to-br to-transparent p-6">
          ...
          <div className="text-warning-soft-foreground text-4xl font-black">
            {coupons?.filter((c) => new Date(c.expireDate) > new Date())
              .length || 0}
          </div>
      ...
      {isLoading ? (
        ...
        <div className="border-border bg-surface rounded-[32px] border p-20 text-center shadow-sm">
          ...
          <p className="text-muted mt-3 text-sm">
            اولین کد تخفیف را ایجاد کنید و مدیریت تخفیف را آغاز کنید.
          </p>
          <Link href="/admin/coupons/create">
            <Button
              color="primary"
              size="lg"
              className="mt-8 rounded-2xl px-8 font-black"
            >
              ساخت اولین کد تخفیف
            </Button>
          </Link>
        ...
        <div className="border-border bg-surface overflow-hidden rounded-[32px] border p-2 shadow-xl shadow-black/5">
          <div className="p-6">
            ...
            <p className="text-muted mt-1 text-xs font-bold">
              مشاهده، ویرایش و حذف کد تخفیف موجود
            </p>
          </div>

          <Table aria-label="جدول کدهای تخفیف">
            <Table.ScrollContainer>
              <Table.Content dir="rtl" className="text-center">
                <Table.Header className="text-nowrap">
                  <Table.Column isRowHeader>#</Table.Column>
                  <Table.Column>کد</Table.Column>
                  <Table.Column>نوع</Table.Column>
                  <Table.Column>مقدار</Table.Column>
                  <Table.Column>شامل محصولات</Table.Column>
                  <Table.Column>تعداد استفاده</Table.Column>
                  <Table.Column>ظرفیت مجاز</Table.Column>
                  <Table.Column>تاریخ انقضا</Table.Column>
                ...
                <Table.Body className="text-nowrap">
                  {coupons?.map((coupon, index) => (
                    <Table.Row key={coupon._id}>
                      ...
                      <Table.Cell className="text-foreground font-black tracking-widest">
                        {coupon.code}
                      </Table.Cell>
                      <Table.Cell>
                        <Chip
                          variant="flat"
                          color={
                            coupon.type === "percent" ? "accent" : "warning"
                          }
                      ...
                      <Table.Cell className="text-foreground font-bold">
                      ...
                      <Table.Cell>
                        <div className="flex flex-wrap items-center justify-center gap-1">
                          {coupon.productIds?.length > 0 ? (
                            coupon.productIds.map((product, idx) => (
                              <Chip
                                key={idx}
                                variant="flat"
                                color="default"
